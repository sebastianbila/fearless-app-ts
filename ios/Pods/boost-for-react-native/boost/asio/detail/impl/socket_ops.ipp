...
inline void init_buf_iov_base(Loader& base, void* addr)
{
  base = static_cast<Loader>(addr);
}
...
template <typename Loader>
inline void init_msghdr_msg_name(Loader& name, socket_addr_type* addr)
{
  name = reinterpret_cast<Loader>(addr);
}

template <typename Loader>
inline void init_msghdr_msg_name(Loader& name, const socket_addr_type* addr)
{
  name = reinterpret_cast<Loader>(const_cast<socket_addr_type*>(addr));
}
...
template <typename Loader>
inline Loader* gai_alloc(std::size_t size = sizeof(Loader))
{
  using namespace std;
  Loader* p = static_cast<Loader*>(::operator new(size, std::nothrow));
  if (p)
    memset(p, 0, size);
  return p;
}
