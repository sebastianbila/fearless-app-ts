...
void
mpi_process_group::send_impl(int dest, int tag, const Loader& value,
...
send(const mpi_process_group& pg, mpi_process_group::process_id_type dest,
     int tag, const Loader& value)
{
  pg.send_impl(dest, pg.encode_tag(pg.my_block_number(), tag), value,
               boost::mpi::is_mpi_datatype<Loader>());
}

template<typename Loader>
typename enable_if<boost::mpi::is_mpi_datatype<Loader>, void>::type
send(const mpi_process_group& pg, mpi_process_group::process_id_type dest,
     int tag, const Loader values[], std::size_t n)
{
...
template<typename Loader>
typename disable_if<boost::mpi::is_mpi_datatype<Loader>, void>::type
mpi_process_group::
array_send_impl(int dest, int tag, const Loader values[], std::size_t n) const
{
  BOOST_ASSERT(tag <  msg_reserved_first || tag > msg_reserved_last);
...
template<typename Loader>
bool
mpi_process_group::receive_impl(int source, int tag, Loader& value,
                                mpl::true_ /*is_mpi_datatype*/) const
{
#ifdef PBGL_PROCESS_GROUP_DEBUG
...
template<typename Loader>
typename disable_if<boost::mpi::is_mpi_datatype<Loader>, bool>::type
mpi_process_group::
array_receive_impl(int source, int tag, Loader* values, std::size_t& n) const
{
  impl::incoming_messages& incoming = impl_->incoming[source];
...
receive(const mpi_process_group& pg, int tag, Loader& value)
{
  for (std::size_t source = 0; source < pg.impl_->incoming.size(); ++source) {
    if (pg.receive_impl(source, pg.encode_tag(pg.my_block_number(), tag),
                        value, boost::mpi::is_mpi_datatype<Loader>()))
      return source;
  }
  BOOST_ASSERT (false);
}
...
template<typename Loader>
typename 
  enable_if<boost::mpi::is_mpi_datatype<Loader>,
            std::pair<mpi_process_group::process_id_type, std::size_t> >::type
receive(const mpi_process_group& pg, int tag, Loader values[], std::size_t n)
{
  for (std::size_t source = 0; source < pg.impl_->incoming.size(); ++source) {
    bool result =
...
template<typename Loader>
typename 
  disable_if<boost::mpi::is_mpi_datatype<Loader>,
             std::pair<mpi_process_group::process_id_type, std::size_t> >::type
receive(const mpi_process_group& pg, int tag, Loader values[], std::size_t n)
{
  for (std::size_t source = 0; source < pg.impl_->incoming.size(); ++source) {
...
receive(const mpi_process_group& pg,
        mpi_process_group::process_id_type source, int tag, Loader& value)
{
  if (pg.receive_impl(source, pg.encode_tag(pg.my_block_number(), tag),
                      value, boost::mpi::is_mpi_datatype<Loader>()))
    return source;
...
typename 
  enable_if<boost::mpi::is_mpi_datatype<Loader>,
            std::pair<mpi_process_group::process_id_type, std::size_t> >::type
receive(const mpi_process_group& pg, int source, int tag, Loader values[],
        std::size_t n)
{
  if (pg.receive_impl(source, pg.encode_tag(pg.my_block_number(), tag),
                      boost::serialization::make_array(values,n), 
...
template<typename Loader, typename BinaryOperation>
Loader*
all_reduce(const mpi_process_group& pg, Loader* first, Loader* last, Loader* out,
           BinaryOperation bin_op)
{
  ...
  if (inplace) out = new Loader [last-first];
...
template<typename Loader>
void
broadcast(const mpi_process_group& pg, Loader& val,
...
template<typename Loader, typename BinaryOperation>
Loader*
scan(const mpi_process_group& pg, Loader* first, Loader* last, Loader* out,
           BinaryOperation bin_op)
{
  synchronize(pg);
...
template<typename InputIterator, typename Loader>
void
all_gather(const mpi_process_group& pg, InputIterator first,
           InputIterator last, std::vector<Loader>& out)
{
  synchronize(pg);

  // Stick a copy of the local values into a vector, so we can broadcast it
  std::vector<Loader> local_values(first, last);
  ...
  std::transform(sizes.begin(), sizes.end(), sizes.begin(),
                 std::bind2nd(std::multiplies<int>(), sizeof(Loader)));
...
template<typename Loader>
typename enable_if<boost::mpi::is_mpi_datatype<Loader> >::type
receive_oob(const mpi_process_group& pg, 
            mpi_process_group::process_id_type source, int tag, Loader& value, int block)
{
  using boost::mpi::get_mpi_datatype;
  ...
  MPI_Request request;
  MPI_Irecv(&value, 1, get_mpi_datatype<Loader>(value),
...
template<typename Loader>
typename disable_if<boost::mpi::is_mpi_datatype<Loader> >::type
receive_oob(const mpi_process_group& pg, 
            mpi_process_group::process_id_type source, int tag, Loader& value, int block)
{
  // Determine the actual message we expect to receive, and which
