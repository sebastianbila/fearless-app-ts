...
//
///////////////////////////////////////////////////////////////////////////////
    template <typename Loader, typename CharT>
    struct tst_node
    {
        ...
        clone() const
        {
            ...
            {
                std::auto_ptr<Loader> mid_data(new Loader(*middle.data));
    ...
    public:
        ...
        Loader* add(IteratorT first, IteratorT const& last, Loader const& data)
        {
            if (first == last)
                return 0;
                ...
                else
                {
                    if (ch == (**np).value)
                    {
                        if (ch == 0)
                        {
                            if ((**np).middle.data == 0)
                            {
                                (**np).middle.data = new Loader(data);
    ...
    private:
        ...
        typedef tst_node<Loader, CharT> node_t;
        node_t* root;
    };
