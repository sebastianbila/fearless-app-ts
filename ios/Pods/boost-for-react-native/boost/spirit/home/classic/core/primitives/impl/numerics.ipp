...
BOOST_SPIRIT_CLASSIC_NAMESPACE_BEGIN
    ...
    namespace impl
    {
        ...
        //
        //        template<typename CharT> static bool digit(CharT ch, Loader& val);
        //
        ///////////////////////////////////////////////////////////////////////
        ...
        {
            template<typename CharT, typename Loader>
            static bool digit(CharT ch, Loader& val)
            {
                val = ch - '0';
                return ('0' == ch || '1' == ch);
            }
        ...
        //          Checking is only performed when std::numeric_limits<Loader>::
...
#pragma warning(disable:4127) //conditional expression is constant
        ...
        template <typename Loader, int Radix>
        struct positive_accumulate
        {
            //  Use this accumulator if number is positive
            static bool add(Loader& n, Loader digit)
            {
                if (std::numeric_limits<Loader>::is_specialized)
                {
                    static Loader const max = (std::numeric_limits<Loader>::max)();
                    static Loader const max_div_radix = max/Radix;
        ...
        struct negative_accumulate
        {
            ...
            static bool add(Loader& n, Loader digit)
            {
                if (std::numeric_limits<Loader>::is_specialized)
                {
                    typedef std::numeric_limits<Loader> num_limits;
                    static Loader const min =
                        (!num_limits::is_integer && num_limits::is_signed && num_limits::has_denorm) ?
                        -(num_limits::max)() : (num_limits::min)();
                    static Loader const min_div_radix = min/Radix;
        ...
        struct extract_int
        {
            template <typename ScannerT, typename Loader>
            static bool
            f(ScannerT& scan, Loader& n, std::size_t& count)
            {
                std::size_t i = 0;
                Loader digit;
        ...
        ///////////////////////////////////////////////////////////////////////
        template <
            typename Loader = unsigned,
            int Radix = 10,
            unsigned MinDigits = 1,
            int MaxDigits = -1
        >
        struct uint_parser_impl
            : parser<uint_parser_impl<Loader, Radix, MinDigits, MaxDigits> >
        {
            typedef uint_parser_impl<Loader, Radix, MinDigits, MaxDigits> self_t;

            template <typename ScannerT>
            struct result
            {
                typedef typename match_result<ScannerT, Loader>::type type;
            };
            ...
            parse(ScannerT const& scan) const
            {
                if (!scan.at_end())
                {
                    ...
                    if (extract_int<Radix, MinDigits, MaxDigits,
                        positive_accumulate<Loader, Radix> >::f(scan, n, count))
                    {
        ...
        struct int_parser_impl
            : parser<int_parser_impl<Loader, Radix, MinDigits, MaxDigits> >
        {
            typedef int_parser_impl<Loader, Radix, MinDigits, MaxDigits> self_t;
            ...
            parse(ScannerT const& scan) const
            {
                typedef extract_int<Radix, MinDigits, MaxDigits,
                    negative_accumulate<Loader, Radix> > extract_int_neg_t;
                typedef extract_int<Radix, MinDigits, MaxDigits,
                    positive_accumulate<Loader, Radix> > extract_int_pos_t;
        ...
        //
        ///////////////////////////////////////////////////////////////////////
        template <typename RT, typename Loader, typename RealPoliciesT>
        struct real_parser_impl
        {
            typedef real_parser_impl<RT, Loader, RealPoliciesT> self_t;
            ...
            RT parse_main(ScannerT const& scan) const
            {
                ...
                Loader               n = n_match.has_valid_attribute() ?
                                    n_match.value() : Loader(0);
...
#endif
                        hit.value(hit.value()
                            * pow(Loader(10), Loader(-hit.length())));
                        if (neg)
                            n -= hit.value();
                        else
                            n += hit.value();
                ...
                if (e_hit)
                {
                    ...
                    if (RT e_n_hit = RealPoliciesT::parse_exp_n(scan))
                    {
                        ...
                        using namespace std;    // allow for ADL to find pow()
#endif
                        n *= pow(Loader(10), Loader(e_n_hit.value()));
