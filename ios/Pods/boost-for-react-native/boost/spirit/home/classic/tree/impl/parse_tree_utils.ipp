...
inline tree_node<Loader> const &
get_first_leaf (tree_node<Loader> const &node)
{
...
template <typename Loader>
inline bool
find_node (tree_node<Loader> const &node, parser_id node_to_search,
    tree_node<Loader> const **found_node)
{
    ...
    if (node.children.size() > 0) {
        typedef typename tree_node<Loader>::const_tree_iterator const_tree_iterator;
...
template <typename Loader>
inline bool
get_node_range (typename tree_node<Loader>::const_tree_iterator const &start,
    parser_id node_to_search,
    std::pair<typename tree_node<Loader>::const_tree_iterator,
        typename tree_node<Loader>::const_tree_iterator> &nodes)
{
// look at this node first
tree_node<Loader> const &node = *start;
...
// look at subnodes now
    if (node.children.size() > 0) {
        ...
        for (const_tree_iterator it = node.children.begin(); it != end; ++it)
        {
            if (impl::get_node_range<Loader>(it, node_to_search, nodes))
                return true;
        }
    }
    return false;
...
template <typename Loader>
inline bool
get_node_range (tree_node<Loader> const &node, parser_id node_to_search,
    std::pair<typename tree_node<Loader>::const_tree_iterator,
        typename tree_node<Loader>::const_tree_iterator> &nodes)
{
    if (node.children.size() > 0) {
