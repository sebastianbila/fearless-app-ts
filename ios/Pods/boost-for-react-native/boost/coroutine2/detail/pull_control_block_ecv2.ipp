...
void
pull_coroutine< Loader >::control_block::destroy( control_block * cb) noexcept {
    boost::context::execution_context< Loader * > ctx = std::move( cb->ctx);
...
pull_coroutine< Loader >::control_block::control_block( context::preallocated palloc, StackAllocator salloc,
                                                   Fn && fn) :
#if defined(BOOST_NO_CXX14_GENERIC_LAMBDAS)
    ctx{ std::allocator_arg, palloc, salloc,
        std::move(
         std::bind(
             [this]( typename std::decay< Fn >::type & fn_, boost::context::execution_context< Loader * > && ctx, Loader *) mutable {
                // create synthesized push_coroutine< Loader >
                typename push_coroutine< Loader >::control_block synthesized_cb{ this, ctx };
                push_coroutine< Loader > synthesized{ & synthesized_cb };
...
#else
    ctx{ std::allocator_arg, palloc, salloc,
         [this,fn_=std::forward< Fn >( fn)]( boost::context::execution_context< Loader * > && ctx, Loader *) mutable {
            // create synthesized push_coroutine< Loader >
            typename push_coroutine< Loader >::control_block synthesized_cb{ this, ctx };
            push_coroutine< Loader > synthesized{ & synthesized_cb };
            other = & synthesized_cb;
...
template< typename Loader >
pull_coroutine< Loader >::control_block::control_block( typename push_coroutine< Loader >::control_block * cb,
                                                   boost::context::execution_context< Loader * > & ctx_) noexcept :
    ctx{ std::move( ctx_) },
    other{ cb },
    state{ state_t::none },
    except{},
...
template< typename Loader >
pull_coroutine< Loader >::control_block::~control_block() {
    // destroy data if set
    if ( bvalid) {
        reinterpret_cast< Loader * >( std::addressof( storage) )->~Loader();
    }
}

template< typename Loader >
void
pull_coroutine< Loader >::control_block::deallocate() noexcept {
...
template< typename Loader >
void
pull_coroutine< Loader >::control_block::resume() {
...
template< typename Loader >
void
pull_coroutine< Loader >::control_block::set( Loader * t) {
    // destroy data if set
    if ( bvalid) {
        reinterpret_cast< Loader * >( std::addressof( storage) )->~Loader();
    }
    if ( nullptr != t) {
        ::new ( static_cast< void * >( std::addressof( storage) ) ) Loader( std::move( * t) );
        bvalid = true;
    } else {
        bvalid = false;
    }
...
template< typename Loader >
Loader &
pull_coroutine< Loader >::control_block::get() noexcept {
    return * reinterpret_cast< Loader * >( std::addressof( storage) );
}

template< typename Loader >
bool
pull_coroutine< Loader >::control_block::valid() const noexcept {
