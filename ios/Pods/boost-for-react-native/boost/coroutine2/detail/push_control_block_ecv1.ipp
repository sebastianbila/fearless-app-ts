         ...
         std::bind(
                 [this]( typename std::decay< Fn >::type & fn_, boost::context::execution_context & ctx, void * vp) mutable noexcept {
                     ...
                     if ( state_t::none == ( state & state_t::destroy) ) {
                         try {
                             // jump back to ctor
                             Loader * t = static_cast< Loader * >( ctx() );
...
template< typename Loader >
push_coroutine< Loader >::control_block::~control_block() {
...
template< typename Loader >
void
push_coroutine< Loader >::control_block::resume( Loader const& t) {
    other->ctx = boost::context::execution_context::current();
    // pass an pointer to other context
    ctx( const_cast< Loader * >( & t) );
    if ( except) {
...
push_coroutine< Loader & >::control_block::resume( Loader & t) {
    other->ctx = boost::context::execution_context::current();
    // pass an pointer to other context
    ctx( const_cast< typename std::remove_const< Loader >::type * >( std::addressof( t) ) );
    if ( except) {
        std::rethrow_exception( except);
    }
}
