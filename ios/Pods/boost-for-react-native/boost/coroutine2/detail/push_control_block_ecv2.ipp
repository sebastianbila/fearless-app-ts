...
void
push_coroutine< Loader >::control_block::destroy( control_block * cb) noexcept {
...
push_coroutine< Loader >::control_block::control_block( context::preallocated palloc, StackAllocator salloc,
                                                   Fn && fn) :
#if defined(BOOST_NO_CXX14_GENERIC_LAMBDAS)
    ctx{ std::allocator_arg, palloc, salloc,
        std::move( 
         std::bind(
             [this]( typename std::decay< Fn >::type & fn_, boost::context::execution_context< Loader * > && ctx, Loader * data) mutable {
                // create synthesized pull_coroutine< Loader >
                typename pull_coroutine< Loader >::control_block synthesized_cb{ this, ctx };
                pull_coroutine< Loader > synthesized{ & synthesized_cb };
...
#else
    ctx{ std::allocator_arg, palloc, salloc,
         [this,fn_=std::forward< Fn >( fn)]( boost::context::execution_context< Loader * > && ctx, Loader * data) mutable {
            // create synthesized pull_coroutine< Loader >
            typename pull_coroutine< Loader >::control_block synthesized_cb{ this, ctx };
            pull_coroutine< Loader > synthesized{ & synthesized_cb };
            other = & synthesized_cb;
...
template< typename Loader >
push_coroutine< Loader >::control_block::control_block( typename pull_coroutine< Loader >::control_block * cb,
...
template< typename Loader >
void
push_coroutine< Loader >::control_block::deallocate() noexcept {
...
template< typename Loader >
void
push_coroutine< Loader >::control_block::resume( Loader const& data) {
    // pass an pointer to other context
    auto result = ctx( const_cast< Loader * >( & data) );
...
template< typename Loader >
void
push_coroutine< Loader >::control_block::resume( Loader && data) {
    // pass an pointer to other context
...
template< typename Loader >
bool
push_coroutine< Loader >::control_block::valid() const noexcept {
...
template< typename Loader >
void
push_coroutine< Loader & >::control_block::resume( Loader & t) {
    // pass an pointer to other context
    auto result = ctx( const_cast< typename std::remove_const< Loader >::type * >( std::addressof( t) ) );
    ctx = std::move( std::get< 0 >( result) );
    if ( except) {
        std::rethrow_exception( except);
    }
